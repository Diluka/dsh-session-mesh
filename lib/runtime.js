...
export class SessionMeshRuntime {
  ...
  async sendSessionMessage(args, caller, signal) {
    ...
    return {
      ...
      accepted: true,
      mode,
      to: this.projectSession(
        { header: /** @type {SessionHeaderLike} */ (agent.session.header), live: true, persisted: false },
        this.workspaceIndex(),
        new Map([[String(agent.id), agent]]),
        senderAgent,
      ),
      from,
      deliveredVia,
      ...
      resume = (async () => {
        ...
        return this.agentRegistry().resume({
          ...
          setup: async (agentCtx, agent) => {
            const projections = /** @type {SessionProjectionsLike | undefined} */ (serviceOf(this.ctx, 'sessionProjections'))
            const selectedPreset = projections?.stateOf(agent.session, 'agentPreset') ?? agentPreset
            const composition = await this.composeAgent(selectedPreset)
