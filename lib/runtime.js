...
/**
 * @param {{ header: SessionHeaderLike, events: readonly SessionEventLike[] }} session
...
function resolveRecordedPreset(session) {
  for (let index = session.events.length - 1; index >= 0; index -= 1) {
    ...
    const data = event && typeof event.data === 'object' && event.data !== null
      ? /** @type {{ agentPreset?: unknown }} */ (event.data)
      : undefined
...
export class SessionMeshRuntime {
  ...
  async currentSession(caller, signal) {
    ...
    return this.projectSession({ header: /** @type {SessionHeaderLike} */ (agent.session.header), live: true, persisted: false }, this.workspaceIndex(), liveAgents, agent)
  }
   ...
   */
  async resolvedPresetForSession(sessionId, fallback) {
    const query = /** @type {SessionQueryLike | undefined} */ (serviceOf(this.ctx, 'sessionQuery'))
   ...
   * @returns {Promise<SessionRow[]>}
   */
  async withUpdatedAt(rows, query) {
    const listEvents = query.listEvents
    if (listEvents === undefined || rows.length === 0) return rows
    return Promise.all(rows.map(async (row) => {
      try {
        const events = await listEvents(row.sessionId)
