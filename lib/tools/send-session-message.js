...
export function buildSendSessionMessageTool(runtime) {
  return createSessionMeshTool({
    ...
    parameters: {
      ...
      properties: {
        sessionId: { type: 'string', description: 'Target ordinary DSH session id. For a relay reply, use dsh-relay.fromSessionId.' },
        message: { type: 'string', description: 'Message body to send after the generated dsh-relay envelope.' },
        summary: { type: 'string', description: 'Optional 5-10 word recap shown by the sender tool card and relay thread index.' },
        ...
        inReplyTo: { type: 'string', description: 'Relay message id this message replies to. For a relay reply, use dsh-relay.messageId.' },
        threadId: { type: 'string', description: 'Relay thread id to continue. For a relay reply, use dsh-relay.threadId; omit it to start a new thread.' },
      },
    },
    outputSchema: {
      type: 'object',
