...
/**
 * @template Args
 * @typedef {object} SessionMeshToolSpec
 * @property {string} name
 * @property {string} description
 * @property {Record<string, unknown>} parameters
 * @property {Record<string, unknown>} outputSchema
 * @property {DshToolDefinition['output']['render']} render
 * @property {(rawArgs: unknown) => Args} parse
 * @property {(args: Args, exec: DshToolRunContext) => Promise<unknown>} run
 * @property {DshToolDefinition['isConcurrencySafe']} [isConcurrencySafe]
 */

/**
 * @template Args
 * @param {SessionMeshToolSpec<Args>} spec
 * @returns {DshToolDefinition}
 */
export function createSessionMeshTool(spec) {
  /** @type {DshToolDefinition} */
  const definition = {
    name: spec.name,
    description: spec.description,
    parameters: /** @type {DshToolDefinition['parameters']} */ (spec.parameters),
    output: {
      schema: /** @type {DshToolDefinition['output']['schema']} */ (spec.outputSchema),
      render: spec.render,
    },
    execute(rawArgs, exec) {
      return spec.run(spec.parse(rawArgs), exec)
    },
  }
  if (spec.isConcurrencySafe !== undefined) definition.isConcurrencySafe = spec.isConcurrencySafe
  return definition
}
